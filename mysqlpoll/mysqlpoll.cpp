...
// 初始化
mysqlpoll::init(std::string host,std::string dbname,std::string dbpasswd,int port,std::string dbname, int connection_num)
: host(host),dbname (dbname), dbpasswd(dbpasswd),port(port),dbname(dbname),max_size(connection_num){
    for(int i = 0; i<connection_num; ++i){
        ...
        con = mysql_real_connect(con,host.c_str(),dbname.c_str(),dbpasswd.c_str(),dbname.c_str(),port.c_str(),NULL,0);
        if(con == NULL){
...
mysqlconnection::~mysqlconnection(){
    mysqlpoll::getInstance()->destory(mysql_);
}
